...
class AppointmentsController {
  ...
  async store(req, res) {
    ...
    if (req.userId === provider_id) {
      return res.status(400).json({ error: 'You not can schedule with yourself' });
    }
    ...
    if (!checkIsProvider) {
      return res
        .status(401)
        .json({ error: 'You can only create appointments providers' });
    }
  ...
  async delete(req, res) {
    ...
    if (appointment.user_id !== req.userId) {
      return res.status(401).json({ error: "You don't permission to cancel this appointment." });
    }
